...
// Shared mutable route object — useRoute() returns this reference
const mockRoute = { path: '/', name: 'index', query: {}, params: {} }
vi.mocked(useRoute).mockReturnValue(mockRoute)

// Timeline route configs matching the test routes
...
// Helper: update mock route to simulate navigation
function setMockRoute(path, name) {
  mockRoute.path = path
  mockRoute.name = name
}
...
describe('useAPI composable', () => {
  ...
  beforeEach(() => {
    ...
    vi.mocked(useNuxtApp).mockReturnValue({ $timeline: mockTimeline })
    vi.mocked(useRoute).mockReturnValue(mockRoute)
    setMockRoute('/', 'welcome_anonymous')
  ...
  it('should check if next and previous views are available', async () => {
    // Set mock route to landing (which has both prev and next)
    setMockRoute('/landing', 'landing')

    expect(api.hasNextView()).toBe(true)
    expect(api.hasPrevView()).toBe(true)

    // Set mock route to test (which has prev but no next)
    setMockRoute('/test', 'test')
  ...
  it('should get URL helpers correctly', () => {
    // New impl uses deployBasePath from runtimeConfig
    expect(api.getPublicUrl('test.png')).toContain('test.png')
  })
  ...
  describe('recordPageData', () => {
    ...
    it('should log error when no route name available', () => {
      ...
      // Set route to something the timeline doesn't know about
      setMockRoute('/unknown', undefined)
  ...
  describe('deprecated methods', () => {
    ...
    it('should reject arrays at top level (prevents nested arrays)', async () => {
      setMockRoute('/landing', 'landing')
  ...
  describe('computeCompletionCode', () => {
    ...
    it('should append status suffix based on completion state', () => {
      ...
      api.store.cookieState.done = true
      api.store.cookieState.withdrawn = false
      const completedCode = api.computeCompletionCode()
      expect(completedCode.endsWith('oo')).toBe(true)

      // Test withdrawn state
      api.store.cookieState.done = false
      api.store.cookieState.withdrawn = true
