...
describe('Timeline middleware guards', () => {
  ...
  describe('Force navigate / dev mode', () => {
    ...
    it('should allow any navigation in presentation mode (via skipBlockingGuards)', async () => {
      // Presentation mode uses skipBlockingGuards (set by middleware prefix detection),
      // not config.mode — Vite MODE is always 'development' or 'production'.
      const result = await executeGuards(
        mockRoute('/instructions'),
        mockRoute('/welcome'),
  ...
  describe('skipBlockingGuards (dev/presentation mode)', () => {
    ...
    it('should enforce pinned route guard even with skipBlockingGuards', async () => {
      store.config.mode = 'development'
      store.dev.pinnedRoute = 'demograph'

      // Pinned route runs before skipBlockingGuards so it still enforces
      const result = await executeGuards(
        mockRoute('/instructions'),
        mockRoute('/welcome'),
        { timeline, store, log, api, skipBlockingGuards: true },
